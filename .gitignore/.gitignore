...
def _combineBins(temp_cont, target):
    ...
    for i in temp_cont.index:
        ...
        if i == temp_cont.index.max():
            il = temp_cont[temp_cont.index<i].index.max()
        else:
            il = temp_cont[temp_cont.index>i].index.min()
        if any(rowdata[:3] == 0):#如果0,1,total有一项为0，则运行
            #
            temp_cont.ix[il, target] = temp_cont.ix[il, target] + rowdata[target]
            temp_cont.ix[il, 0] = temp_cont.ix[il, 0] + rowdata[0]
            temp_cont.ix[il, 'total'] = temp_cont.ix[il, 'total'] + rowdata['total']
            #
            if i < temp_cont.index.max():
                temp_cont.ix[il,'lower'] = rowdata['lower']
            else:
                temp_cont.ix[il,'upper'] = rowdata['upper']
